...
<section id="diningSection" style="padding-top: 17px;">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <a class="diningHeading">Royena Stars</a>
            ...
            <div class="col-md-6  m-4 p-5" style="background: #f1f1f1;">
                <div class="loyaltyItem">
                    <a class="diningHeading" style="    color: #09203b; font-size: 25px;font-weight: 600;">
                        {{$with_title->title}}:</a>
                    <h5 class="mt-3 loyaltyServiceHeading"> <i class="fa-brands fa-servicestack"></i> {{$with_title->second_title}}:</h5>
                    <?php
                    $items = explode(',', $with_title->description);
                    foreach ($items as $list):
                    ?>
                        <li><i class="<?= $with_title->icon ? $with_title->icon : 'fa-solid fa-check' ?>"></i> <span style="margin-left: 20px;"> {{$list}}.</span></li>
                    <?php endforeach ?>
                ...
                <div class="loyaltyItem">
                    <h5 class="mt-3 loyaltyServiceHeading"> <i class="fa-brands fa-servicestack"></i> {{$data->second_title}}:</h5>
                    <?php
                    $items = explode(',', $data->description);
                    ?>
                    @foreach($items as $list)
                    <li><i class="<?= $data->icon ? $data->icon : 'fa-solid fa-check' ?>"></i> <span style="margin-left: 20px;"> {{$list}}.</span></li>
                    @endforeach
                    <!-- <li><i class="fa-solid fa-check"></i> <span style="margin-left: 20px;"> 8 Nights stay on Weekdays for 2 per persons except blackout dates & public.</span></li> -->
                </div>
                @endforeach
